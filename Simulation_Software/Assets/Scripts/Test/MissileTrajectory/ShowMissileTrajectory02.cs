    ...
    void LoadTrajectoryData()
    {
        ...
        foreach (string filePath in missileFiles)
        {
            ...
            for (int i = 1; i < lines.Length; i++) // 跳过表头
            {
                ...
                if (cols.Length < 4) // 检查列数：时间, x, y, z
                {
                    Debug.LogWarning($"文件 {filePath} 中第 {i} 行的列数不足，跳过该行。");
                    continue;
                }

                float time = float.Parse(cols[0]);
                float x = -float.Parse(cols[1]) / 250; // 第二列是x
                float y = float.Parse(cols[2]) / 900; // 第三列是y
                float z = float.Parse(cols[3]) / 450;  // 第四列是z
